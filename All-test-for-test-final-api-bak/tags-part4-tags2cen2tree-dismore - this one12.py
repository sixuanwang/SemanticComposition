...
inputf = file('tags-init-internet.txt')
outputf = file('tags-centrality-allsteps-with-threthold1w1q&lev-in-distance&internet12.txt', 'w')
outputf2 = file('tags-tree-allsteps&ration1w1q&lev-in-distance&internet12.txt', 'w')
lineid = 0
...
import Synonym
#import Synonym-worknet
SYNONYM_YES_RATIO = 1.0
SYNONYM_NO_RATIO = 0.6	
...
for tag_item in myTagSet:
	...
	for pair_item in myTagPairs: #get related pairs that contains tag1
		...
		if (lev2 == 0.0):
			...
			likelihoodOfPair = 1.0/((1.0)*myTagPairs[pair_item]*lev2) # distance = 1/(frequency * levenshtein * synonym)  ######
...
INIT_SIMILARITY_IN_GRAPH = 10000.0
THRESHOLD_ADD_TREE_NOTE = 1000.0 # whether close enough, the smaller the better
#RATIO_DISTANCE = 0.5 #no need ratio, bacause we are multiplying
#RATIO_LEVENSHTEIN = 0.5
RATIO_SYNONYM = 0.0
	...
	for tagInCentrality in centrality: #tagCandidate[0] is tag that wants to add in the tree, WARNING: should be tagCandidate
		...
		if(treeTags): #when tree has tags rather than root
			for tagInTree in treeTags: #find a closest one in the tree
				...
				similarity_new = graph_distance(tagCandidate,tagInTree)
