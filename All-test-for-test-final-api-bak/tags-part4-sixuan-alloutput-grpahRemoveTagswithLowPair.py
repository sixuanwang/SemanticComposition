...
outputfTag = file('tags-tag-140-short-s.txt', 'w')
outputfGrapgh = file('tags-graph-140-short-s.txt', 'w')
outputf = file('tags-centrality-140-short-s.txt', 'w')
outputf2 = file('tags-tree-140-shorts-s.txt', 'w')

inputf = 'taginfo_parsed_1k.txt'
outputfTag = file('tags-tag-140-1k-s.txt', 'w')
outputfGrapgh = file('tags-graph-140-1k-s.txt', 'w')
outputf = file('tags-centrality-140-1k-s.txt', 'w')
outputf2 = file('tags-tree-140-1k-s.txt', 'w')

inputf = 'taginfo_parsed_1k.txt'
outputfTag = file('tags-tag-140-1k-s2.txt', 'w')
outputfGrapgh = file('tags-graph-140-1k-s2.txt', 'w')
outputf = file('tags-centrality-140-1k-s2.txt', 'w')
outputf2 = file('tags-tree-140-1k-s2.txt', 'w')
'''

#files to save
inputf = 'taginfo_parsed_1k.txt'
outputfTag = file('tags-tag-140-1k-s-gfrm-p500.txt', 'w')
outputfGrapgh = file('tags-graph-140-1k-s-gfrm-p500.txt', 'w')
outputf = file('tags-centrality-140-1k-s-gfrm-p500.txt', 'w')
outputf2 = file('tags-tree-140-1k-s-gfrm-p500.txt', 'w')
...
Threthold_Tag_Pair = 5                # remove the tag pairs with lower frequency
...
THRESHOLD_ADD_TREE_NOTE = 0.1         # whether close enough; if lower, child of root; 
RATIO_LEVENSHTEIN = 0.5               # Similarity_Syntactic; the higher, the more significant
RATIO_SYNONYM = 0.75                   # Similarity_Semantic from wordnet; the higher, the more significant
...
def read_APIs(line):	
	...
	for wordList in lineList:
		...
		if len(wordList) == 0:
			break;
		#if len(wordList) == 1: #filter the wordList with single letter
		#	break;
		if wordList == '\n':
			break;		
		...
		if(len(words)==2):
			tag_name = words[0].strip()
			tag_fre = int(words[1].strip())	
							
			#for counting in( TagSet
			if tag_name in myTagSet.keys():
				myTagSet[tag_name] += tag_fre
			else:
				myTagSet[tag_name] = tag_fre
...
def read_APIs_again(line):	
	...
	for wordList in lineList:
		...
		if(len(words)==2):
			...
			if tag_name in myTagSet.keys():  # only save this tag for our API if existed in tagSet
				if tag_name in readinglist.keys():
					readinglist[tag_name] += tag_fre
				else:
					readinglist[tag_name] = tag_fre
	...
	for tagInCentrality in centrality: #tagCandidate[0] is tag that wants to add in the tree, WARNING: should be tagCandidate
		...
		if(treeTags): #when tree has tags rather than root
			for tagInTree in treeTags: #find a closest one in the tree
				...
				similarity_new = similarity_tags_sixuan(tagCandidate,tagInTree)
